...
def init_db():
    ...
    # Universal Migration: Fix hashes for ALL users (including those added via UI)
    all_users = cursor.execute("SELECT id, username, password, role FROM users").fetchall()
    
    # Map roles to default passwords for migration if we don't know the plain text
    # This is a bit of a hack but necessary if we can't parse the old hash
    role_password_map = {
        'admin': 'admin123',
        'patient': 'patient123',
    ...
    for user in all_users:
        if user['password'].startswith('$2y$'):
            # It's a legacy hash. We can't decrypt it, so we reset to a default based on role
            # or try to find if it was one of our defaults.
            ...
            print(f"Migrated legacy hash for user: {user['username']} (Reset password to default for {user['role']})")
